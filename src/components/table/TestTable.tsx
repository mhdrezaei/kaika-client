        ...
        {/* Rtable-row Rtable-row--head  = > display:none*/}
           ...
           {headers.map((element , index) =>(
            // Rtable-cell date-cell column-heading
               <div key={index} className='w-full flex items-center md:flex-grow md:py-2 md:px4 md:overflow-hidden md:list-none'>
            <Typography variant="small" className="text-base font-medium uppercase text-blue-gray-50">
                        {element}
                      </Typography>
            </div>
           ))}
